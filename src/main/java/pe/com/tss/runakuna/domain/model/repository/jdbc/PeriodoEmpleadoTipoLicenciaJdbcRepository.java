...
public class PeriodoEmpleadoTipoLicenciaJdbcRepository implements PeriodoEmpleadoTipoLicenciaRepository{
	...
	public List<PeriodoEmpleadoTipoLicenciaViewModel> findOneByIdPeriodo(Long idPeriodo) {
		WhereParams params = new WhereParams();
        String sql = findOneByIdPeriodoQuery(idPeriodo, params);

        return jdbcTemplate.query(sql, params.getParams(),
                new BeanPropertyRowMapper<PeriodoEmpleadoTipoLicenciaViewModel>(PeriodoEmpleadoTipoLicenciaViewModel.class));
	}

	private String findOneByIdPeriodoQuery(Long idPeriodo, WhereParams params) {
		StringBuilder sql = new StringBuilder();
        sql.append("  SELECT pe.IdPeriodoEmpleado as idPeriodoEmpleado ");
        sql.append("  FROM ");
        sql.append("  PeriodoEmpleadoTipoLicencia pe ");
        sql.append("  WHERE pe.IdPeriodoEmpleado= "+idPeriodo);
