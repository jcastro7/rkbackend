...
public class HistoriaLaboralJdbcRepository implements HistoriaLaboralRepository{
	...
	private String generarHistoriasLaboralesPorEmpleado(Long idEmpleado, WhereParams params) {
		...
		sql.append("  ep.ApellidoPaterno +' '+ep.ApellidoMaterno+', '+ep.Nombre AS jefeInmediato ");
		sql.append("  FROM ");
		sql.append("  HistorialLaboral h ");
	...
	public EmpleadoCompensacionViewModel obtenerHorasPorCompensarPorEmpleado(Long idEmpleado) {
		WhereParams params = new WhereParams();
		String sql = obtenerHorasPorCompensarPorEmpleadoQuery(idEmpleado, params);
		
		return jdbcTemplate.queryForObject(sql, params.getParams(),
				new BeanPropertyRowMapper<EmpleadoCompensacionViewModel>(EmpleadoCompensacionViewModel.class));
	}

	private String obtenerHorasPorCompensarPorEmpleadoQuery(Long idEmpleado, WhereParams params) {
		StringBuilder sql = new StringBuilder();
		
		sql.append(" SELECT e.HorasPendientesTotal as horasPendientesTotal FROM EmpleadoCompensacion e ");
	    sql.append(" WHERE 1=1 ");
