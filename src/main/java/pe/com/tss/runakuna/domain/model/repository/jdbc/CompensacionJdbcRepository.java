...
public class CompensacionJdbcRepository implements CompensacionRepository {
    ...
    private String searchByFilterQuery(CompensacionFilterViewModel compensacionFilter, WhereParams params) {
        ...
        sql.append(" CONVERT(int,SUBSTRING(EMPCOM.Mes,4,4)) AS numeroAnio, ");
        sql.append(" CONVERT(int,SUBSTRING(EMPCOM.Mes,1,2)) AS numeroMes ");
        ...
        sql.append(params.filter(" AND Convert(int,SUBSTRING(EMPCOM.Mes,1,2)) = :mes ", compensacionFilter.getMes()));
        sql.append(params.filter(" AND Convert(int,SUBSTRING(EMPCOM.Mes,4,4)) = :anio ", compensacionFilter.getAnio()));
        
        sql.append(params.filter(" AND [dbo].[GetJefeInmediato] (EMP.IdEmpleado,HISTORIAL.IdHistorialLaboral) = :idJefeInmediato",compensacionFilter.getIdJefeInmediato()));
        
        sql.append(" ORDER BY numeroAnio DESC, numeroMes DESC, empleado");
	...
	private String generarBusquedaRapidaCompensaciones(CompensacionQuickFilterViewModel quickFilter, WhereParams params) {
        ...
        sql.append(" where 1=1");
        sql.append(" AND Convert(int,SUBSTRING(EMPCOM.Mes,1,2)) = MONTH('"+getdate+"') ");
