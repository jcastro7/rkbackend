...
public interface HorarioEmpleadoJpaRepository extends CrudRepository<HorarioEmpleado, Long>, JpaRepository<HorarioEmpleado, Long>{
	...
	@Query("SELECT e FROM HorarioEmpleado e WHERE e.empleado.idEmpleado = :idEmpleado AND e.inicioVigencia<=:fecha AND(e.finVigencia IS NULL OR (e.finVigencia IS NOT NULL AND e.finVigencia>=:fecha))")
	HorarioEmpleado findOneByIdEmpleadoAndDate(@Param("fecha") Date fecha, @Param("idEmpleado") Long idEmpleado);
