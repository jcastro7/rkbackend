...
public class CargoController extends BaseController{
	...
	public @ResponseBody CargoViewModel obtenerCargoDetalle(@RequestBody Long idCargo){
	 ...
	 @RequestMapping(value = "/obtenerCargoComboHistorialLaboral", method = RequestMethod.POST)
	    public @ResponseBody List<CargoComboViewModel> obtenerCargoComboHistorialLaboral(@RequestBody ProyectoFilterViewModel filter){
	    	List<CargoComboViewModel> result = new ArrayList<>();
			result = cargoService.obtenerCargoComboHistorialLaboral(filter);
