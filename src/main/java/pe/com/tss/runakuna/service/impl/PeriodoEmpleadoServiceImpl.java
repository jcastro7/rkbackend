...
import pe.com.tss.runakuna.domain.model.repository.jdbc.PeriodoEmpleadoTipoLicenciaJdbcRepository;
import pe.com.tss.runakuna.domain.model.repository.jdbc.TipoLicenciaJdbcRepository;
...
public class PeriodoEmpleadoServiceImpl  implements PeriodoEmpleadoService{
	...
	@Autowired
	PeriodoEmpleadoTipoLicenciaJdbcRepository periodoEmpleadoTipoLicenciaJdbcRepository;
	
	@Autowired
	TipoLicenciaJdbcRepository tipoLicenciaJdbcRepository;
	...
	public NotificacionEmpleadoViewModel registrarPeriodoEmpleado(EmpleadoViewModel empleadoDto) {
		...
		try {
			...
			notificacionEmpleadoDto.setSeverity("error");
			notificacionEmpleadoDto.setSummary("Runakuna Error");
	...
	private PeriodoEmpleado crearPeriodoEmpleado(EmpleadoViewModel empleadoDto, Date fechaInicioPeriodo) {
		...
		periodoEmpleado.setDiasVacacionesAcumulado(new Integer(0));
	...
	public List<PeriodoEmpleadoResultViewModel> busquedaPeriodoEmpleado(
		...
		for(PeriodoEmpleadoResultViewModel periodoResult:dto){
			periodoResult.setDiasVacacionesPorVencer(0);
			Long idPeriodo=periodoEmpleadoRepository.obtenerIdUltimoPeriodo(periodoResult.getIdEmpleado());
			if(periodoResult.getIdPeriodoEmpleado().intValue()==idPeriodo){
				periodoResult.setDiasVacacionesPorVencer(periodoResult.getDiasVacacionesDisponibles());
				periodoResult.setDiasVacacionesDisponibles(0);
				
				//info TipoLicencia
//				List<PeriodoEmpleadoTipoLicenciaViewModel> periodoEmplTipoLicencia = periodoEmpleadoTipoLicenciaJdbcRepository.findOneByIdPeriodo(idPeriodo);
//				for(PeriodoEmpleadoTipoLicenciaViewModel next: periodoEmplTipoLicencia){
//					List<TipoLicenciaViewModel> tipoLicencia = tipoLicenciaJdbcRepository.findOneByIdPeriodoEmpleado(next.getIdPeriodoEmpleado());
//					periodoResult.setTipoLicencia(tipoLicencia);
//				}
	...
	public List<PeriodoEmpleadoResultViewModel> obtenerPeriodosConVacacionesDisponibles(Long idEmpleado) {
		List<PeriodoEmpleadoResultViewModel> dto = new ArrayList<PeriodoEmpleadoResultViewModel>();
		dto = periodoEmpleadoRepository.obtenerPeriodosConVacacionesDisponiblesMayor15Dias(idEmpleado);
	...
	@Override
	public List<PeriodoEmpleadoResultViewModel> busquedaPeriodoDisponibleVacaciones(Long idEmpleado) {
		List<PeriodoEmpleadoResultViewModel> dto = new ArrayList<PeriodoEmpleadoResultViewModel>();
		dto = periodoEmpleadoRepository.obtenerPeriodosConVacacionesDisponibles(idEmpleado);
		for(PeriodoEmpleadoResultViewModel periodoResult:dto){
			periodoResult.setDiasVacacionesPorVencer(0);
			Long idPeriodo=periodoEmpleadoRepository.obtenerIdUltimoPeriodo(periodoResult.getIdEmpleado());
			if(periodoResult.getIdPeriodoEmpleado().intValue()==idPeriodo){
	...
	@Override
	public List<PeriodoEmpleadoResultViewModel> busquedaTipoLicencia(Long idPeriodoEmpleado) {
		
		List<PeriodoEmpleadoResultViewModel> dto = new ArrayList<PeriodoEmpleadoResultViewModel>();
		dto = periodoEmpleadoRepository.busquedaPeriodoById(idPeriodoEmpleado);
							
				//info TipoLicencia
				List<PeriodoEmpleadoTipoLicenciaViewModel> periodoEmplTipoLicencia = periodoEmpleadoTipoLicenciaJdbcRepository.findOneByIdPeriodo(idPeriodoEmpleado);
				for(PeriodoEmpleadoResultViewModel periodoResult:dto){
					for(PeriodoEmpleadoTipoLicenciaViewModel next: periodoEmplTipoLicencia){
						List<TipoLicenciaViewModel> tipoLicencia = tipoLicenciaJdbcRepository.findOneByIdPeriodoEmpleado(next.getIdPeriodoEmpleado());
						periodoResult.setTipoLicencia(tipoLicencia);
