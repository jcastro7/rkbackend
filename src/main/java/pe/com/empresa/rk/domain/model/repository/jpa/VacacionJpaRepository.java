...
public interface VacacionJpaRepository extends CrudRepository<Vacacion, Long>, JpaRepository<Vacacion, Long> {
    ...
    @Query("SELECT v FROM Vacacion v WHERE  (v.fechaInicio <= :fecha AND v.fechaFin >= :fecha) " +
            "AND v.periodoEmpleado.idPeriodoEmpleado = :idPeriodoEmpleado")
    Vacacion obtenerVacaciones(@Param("fecha") Date fecha, @Param("idPeriodoEmpleado") Long idPeriodoEmpleado);

    @Query("SELECT v FROM Vacacion v WHERE v.estado != 'R' AND v.periodoEmpleado.idPeriodoEmpleado = :idPeriodoEmpleado" +
            " AND v.idVacacion != :idVacacion")
